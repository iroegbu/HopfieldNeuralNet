    ...
    public class Matrix
    {
        ...
        public double this[int row, int col]
        {
            ...
            set
            {
                ...
                {
                    throw new Exception("Trying to assign invalud number to matrix: "
                            + value);
                }
        ...
        public bool equals(Matrix matrix, int precision)
        {
            ...
            if (precision < 0)
            {
                throw new Exception("Precision can't be a negative number.");
            }

            double test = Math.Pow(10.0, precision);
            if (double.IsInfinity(test) || (test > long.MaxValue))
            {
                throw new Exception("Precision of " + precision
        ...
        public Matrix GetCol(int col)
        {
            if (col > this.Cols)
            {
                throw new Exception("Can't get column #" + col
        ...
        public Matrix GetRow(int row)
        {
            if (row > this.Rows)
            {
                throw new Exception("Can't get row #" + row
                        + " because it does not exist.");
            }
        ...
        private void Validate(int row, int col)
        {
            ...
            {
                throw new Exception("The row:" + row + " is out of range:"
                        + this.Rows);
            }

            if ((col >= this.Cols) || (col < 0))
            {
                throw new Exception("The col:" + col + " is out of range:"
                        + this.Cols);
            }
        }
    }
